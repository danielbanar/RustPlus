...
{
	SDL_Rect mapRect{
		MAP_SIZE_HALF - g.fMapX * g.fMapZoom,
		MAP_SIZE_HALF - g.fMapY * g.fMapZoom,
		MAP_WINDOW_SIZE * g.fMapZoom,
		MAP_WINDOW_SIZE * g.fMapZoom };
...
void Render_Events()
{
	...
	for (int i = 0; i < g.appMapMarkers.markers_size(); i++)
	{
		...
		else if (marker.type() == CH47 || marker.type() == PatrolHelicopter)
		{
			...
			y /= scale;
			SDL_Rect rect{
				MAP_SIZE_HALF - (10.f * g.fMapZoom) + (x - g.fMapX) * g.fMapZoom,
				MAP_SIZE_HALF - (10.f * g.fMapZoom) + (y - g.fMapY) * g.fMapZoom,
				20.f * g.fMapZoom,
				20.f * g.fMapZoom };
			SDL_RenderCopyEx(g.pRenderer, Icons::chinook, 0, &rect, -marker.rotation() - 90.0, 0, SDL_FLIP_NONE);

			SDL_Rect rectBlades1{
				MAP_SIZE_HALF - (6.f * g.fMapZoom) + (x + 6.f * sinf(DEG2RAD(marker.rotation())) - g.fMapX) * g.fMapZoom,
				MAP_SIZE_HALF - (6.f * g.fMapZoom) + (y + 6.f * cosf(DEG2RAD(marker.rotation())) - g.fMapY) * g.fMapZoom,
				12.f * g.fMapZoom,
				12.f * g.fMapZoom };
...
void Render_Markers()
{
	...
	for (int i = 0; i < g.appTeamInfo.leadermapnotes_size(); i++)
	{
		...
		y /= scale;
		SDL_Rect rectMarker{
			MAP_SIZE_HALF - 16.f + (x - g.fMapX) * g.fMapZoom,
			MAP_SIZE_HALF - 32.f + (y - g.fMapY) * g.fMapZoom,
			32.f,
			32.f };
	...
	for (int i = 0; i < g.appTeamInfo.mapnotes_size(); i++)
	{
		...
		SDL_Rect rectMarker{
			MAP_SIZE_HALF - 16.f + (x - g.fMapX) * g.fMapZoom,
			MAP_SIZE_HALF - (teamMarker.type() ? 32.f : 16.f) + (y - g.fMapY) * g.fMapZoom,
			32.f,
			32.f };
...
{
	SDL_Window* window = SDL_CreateWindow("Camera Window", SDL_WINDOWPOS_CENTERED,
		SDL_WINDOWPOS_CENTERED, 160, 90, 0);
	...
	SDL_RenderDrawPoint(cameraRenderer, 100, 50);
